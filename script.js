document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const validateField = function (field) {
      ...
      const labelEl = contactForm.querySelector('label[for="' + field.id + '"]');
      const fieldName = labelEl
        ? labelEl.textContent.trim().replace('*', '').trim()
  ...
  if (animatableElements.length > 0) {
    ...
    const injectAnimationStyles = function () {
      if (document.getElementById('scroll-animate-styles')) return;

      const style = document.createElement('style');
      style.id = 'scroll-animate-styles';
      style.textContent = [
        '.scroll-animate {',
        '  opacity: 0;',
    ...
    if ('IntersectionObserver' in window) {
      ...
      const fallbackScrollHandler = function () {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Torin משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '3fd31500-260c-4b6d-8a91-8781be14b30f',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '3fd31500-260c-4b6d-8a91-8781be14b30f';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '3fd31500-260c-4b6d-8a91-8781be14b30f';
